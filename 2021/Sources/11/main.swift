...
enum ParsingError: Error {
  case invalidPowerLevel(_ raw: Character)
}
...
struct DumboOctopus: Hashable {
  ...
  init(initialPowerDescription: Character) throws {
    guard let power = UInt8("\(initialPowerDescription)") else {
      throw ParsingError.invalidPowerLevel(initialPowerDescription)
    }
    self.power = power
  }
  
  /// increments power by one, returns whether this increment caused a flash
  mutating func increment() -> Bool {
    power += 1
    
    // only flash once per step, i.e., exactly when we hit 10
    return power == 10
  }
...
extension DumboOctopus: CustomStringConvertible {
...
struct World {
  ...
  init(description: String) throws {
    octopuses = try description
      .split(separator: "\n")
      .map { try $0.map { try .init(initialPowerDescription: $0) } }
  }
  
  func neighbors(_ coord: Coordinate) -> [Coordinate] {
    let x = coord.x
    let y = coord.y
    ...
    .compactMap { (x, y) in
      if y >= 0, y < octopuses.count,
         x >= 0, x < octopuses[y].count {
        return .init(x: x, y: y)
      }
      return nil
    }
  ...
  mutating func step() {
    // perform step + ripple effect
    octopuses.indices.forEach { y in
      octopuses[y].indices.forEach { x in
        var workList: Deque<Coordinate> = [.init(x: x, y: y)]
        
        while !workList.isEmpty {
          let coord = workList.removeFirst()
          
          if octopuses[coord.y][coord.x].increment() {
            flashes += 1
            workList.append(contentsOf: neighbors(coord))
          }
        }
      }
    }
...
extension World: CustomStringConvertible {
  var description: String {
    "\(Array(repeating: "-", count: octopuses.first?.count ?? 4).joined())\n"
    + octopuses.map { $0.map { "\($0)" }.joined() }.joined(separator: "\n")
    + "\n\(Array(repeating: "-", count: octopuses.last?.count ?? 4).joined())"
  }
}
