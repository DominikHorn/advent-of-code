...
struct FuelComputer {
  ...
  init(description: String) throws {
    positions = try description.split(separator: ",").map {
      guard let pos = Int($0) else {
        throw ParsingError.invalidPosition(String($0))
      }
      return pos
    }
  }
  
  func linearFuelCost(targetPos: Int) -> Int {
    positions.reduce(0) { aggr, curr in
      aggr + abs(curr - targetPos)
    }
  }
  
  func crabFuelCost(targetPos: Int) -> Int {
    positions.reduce(0) { aggr, curr in
      // euler sum(0...x) = (x*(x+1)) / 2
      let d = abs(curr - targetPos)
      return aggr + (d * (d + 1)) / 2
    }
  }
  
  func cheapestTargetPos(fuelCost: (Int) -> Int) -> (pos: Int, cost: Int) {
    /// trying all fuel costs is n^2 -> bad. Make educated guess and follow gradient
    var pos = Int(avgPos().rounded(.toNearestOrAwayFromZero))
    var currentCost = fuelCost(pos)
    
    while fuelCost(pos+1) < currentCost {
      pos += 1
      currentCost = fuelCost(pos)
    }
    while fuelCost(pos-1) < currentCost {
      pos -= 1
      currentCost = fuelCost(pos)
    ...
    return (pos, currentCost)
  }
  
  private func avgPos() -> Double {
    Double(positions.reduce(0) { $0 + $1 }) / Double(positions.count)
  }
  
  enum ParsingError: Error {
    case invalidPosition(_ raw: String)
  }
}
...
let testComp = try FuelComputer(description: testInput)
print(testComp.cheapestTargetPos(fuelCost: testComp.linearFuelCost(targetPos:)))

let comp = try FuelComputer(description: input)
print(comp.cheapestTargetPos(fuelCost: comp.linearFuelCost(targetPos:)))
print(comp.cheapestTargetPos(fuelCost: comp.crabFuelCost(targetPos:)))
