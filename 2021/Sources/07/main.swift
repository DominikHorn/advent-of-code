...
struct FuelComputer {
  ...
  func fuelCost(targetPos: Int) -> Int {
  ...
  func cheapestTargetPos() -> Int {
    ...
    var currentCost = fuelCost(targetPos: pos)
    
    while fuelCost(targetPos: pos+1) < currentCost {
      pos += 1
      currentCost = fuelCost(targetPos: pos)
    }
    while fuelCost(targetPos: pos-1) < currentCost {
      pos -= 1
      currentCost = fuelCost(targetPos: pos)
...
print(testComp.cheapestTargetPos())

let comp = try FuelComputer(description: input)
print(comp.fuelCost(targetPos: comp.cheapestTargetPos()))
