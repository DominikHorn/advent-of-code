...
struct Point: Hashable {
  ...
  init(description: String) throws {
    let coords = description.split(separator: ",")
    guard coords.count == 2 else {
      throw ParsingError.invalidFormat
    }
    
    guard let x = Int(coords[0]), let y = Int(coords[1]) else {
      throw ParsingError.invalidNumber(description)
    }
  ...
  enum ParsingError: Error {
    case invalidFormat
    case invalidNumber(_ raw: String)
  }
}
...
struct Line: Hashable {
  ...
  init(description: String) throws {
    ...
    self.start = try .init(description: points[0])
    self.end = try .init(description: points[1])
  }
...
struct CoordinateSystem {
  ...
  private mutating func emplace(_ point: Point, of line: Line) {
    var s = points[point] ?? .init()
    s.insert(line)
    points[point] = s
  }
  ...
  init(description: String) throws {
    try description
      .split(separator: "\n")
      .map { try Line(description: String($0)) }
      .forEach { line in
        ...
        try line.coveredPoints().forEach { point in
          emplace(point, of: line)
        }
      }
  }
}

extension CoordinateSystem: CustomStringConvertible {
  var description: String {
    (minY...maxY).map { y in
      (minX...maxX).map { x in
        let cnt = points[.init(x: x, y: y)]?.count ?? 0
        return cnt > 0 ? "\(cnt)" : "."
      }
      .joined()
    }
